...
+ (void)uploadVoice:(NSString *)url parameters:(NSDictionary *)parameters voicesUrl:(NSArray *)voicesUrl
    ...
    NSMutableURLRequest *request = [[AFHTTPRequestSerializer serializer] multipartFormRequestWithMethod:@"POST" URLString:requestUrl parameters:requestParam constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
        // 上传 多张图片
        for (int i = 0; i < voicesUrl.count; i ++) {
            NSString *fileName = [NSString stringWithFormat:@"%@%02d.%@",@"file",i,@"wav"];
            NSURL * url = [NSURL fileURLWithPath:voicesUrl[i]];
            NSData * voiceData = [NSData dataWithContentsOfURL:url];
            SSLog(@"post fileName:%@",fileName);
